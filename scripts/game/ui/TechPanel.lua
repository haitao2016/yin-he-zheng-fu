...
local TIER_NODES = {
    ...
    { "WARP_DRIVE", "ADVANCED_WEAPONS", "DEFENSE_MATRIX" },                                    -- Tier3
    { "QUANTUM_CORE", "PHASE_DRIVE", "NOVA_CANNON", "FORTRESS_PROTOCOL" },                     -- Tier4 P1-1
}
...
for tier, ids in ipairs(TIER_NODES) do
    ...
    local startY    = (4 * ROW_GAP - totalRowH) / 2
...
-- 内容总高（标签行 + 节点行）
local CONTENT_H = HEADER_H + 4 * ROW_GAP + 8
...
-- 辅助：绘制一条贝塞尔前置连线
-- ============================================================================
local function drawConnection(vg, clrFn, x1, y1, x2, y2, r, g, b, a)
...
function TechPanel.Render(ctx)
    ...
    for _, id in ipairs(TECH_ORDER) do
        ...
        for _, pre in ipairs(t.prereqs) do
            ...
            if ppos then
                ...
                -- P1-1: 前置已解锁→主题色，否则深灰
                local unlocked = rs.unlocked[pre]
                local prePos   = ppos
                local tc       = TIER_COLORS[prePos.tier]
                local r2 = unlocked and tc[1] or 55
                local g2 = unlocked and tc[2] or 75
                local b2 = unlocked and tc[3] or 120
                local a2 = unlocked and 160 or 70
    ...
    if selectedId_ and TECHS[selectedId_] then
        ...
        local dvy   = nodesBaseY + 4 * ROW_GAP + 2
