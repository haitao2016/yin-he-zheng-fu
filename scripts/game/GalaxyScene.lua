...
-- P1-1: 星系种子系统
local currentSeed_     = nil       ---@type string|nil  当前星系种子码（6位）
local currentShape_    = nil       ---@type string|nil  当前星系形态
local specialPlanets_  = {}        -- { {planet=ref, type="DEAD"|"STORM"|"BINARY"} }
...
local expeditionPaths_    = {}   -- P1-2: 远征路径数据 [{fleetId,baseId,startX,startY,baseX,baseY,elapsed,duration}]
-- ============================================================================
...
local ASTEROID_SPAWN_WEIGHTS = {
    normal = { minerals=0.42, energy=0.35, crystal=0.15, blueCrystal=0.05, mixed=0.03 },
    ...
    for _, sys in ipairs(starSystems_) do
        for _, p in ipairs(sys.planets) do
            if not p.colonized and not p.isBase then
                ...
                if dist(wx, wy, px, py) < PLANET_CONTACT_DIST then
                    return p
                end
            end
        end
...
local function updateHover()
    ...
    for _, sys in ipairs(starSystems_) do
        for _, p in ipairs(sys.planets) do
            if p._sx and dist(mouseX_, mouseY_, p._sx, p._sy) < (p.size * zoom_ + 10) then
                hoveredPlanet_ = p
                return
            end
        end
...
local function handleClick(mx, my)
    ...
    for _, sys in ipairs(starSystems_) do
        for _, p in ipairs(sys.planets) do
            if p._sx and dist(mx, my, p._sx, p._sy) < (p.size * zoom_ + 12) then
                ...
                if selectedFleetId_ then
                    ...
                    if obj then
                        ...
                        obj.targetPlanet = p   -- P2-4: 记录目标星球（用于星门瞬移判定）
                    ...
                    if p._sx and dist(mx, my, p._sx, p._sy) < (p.size * zoom_ + 12) then
                        ...
                        if selectedFleetId_ then
                            ...
                            if obj then
                                ...
                                obj.targetX = wx + (math.random()-0.5)*40
                                obj.targetY = wy + (math.random()-0.5)*40
                                obj.targetPlanet = p   -- P2-4: 星门瞬移目标
                                if onFleetMove_ then onFleetMove_() end
                            end
                        end
                        return
...
local function updateFleets(dt)
    ...
    for i = 1, fm_.maxFleets do
        ...
        if fl and #fl.ships > 0 then
            ...
            if obj.targetX then
                -- P2-4: 星门节点瞬移检测
                if obj.targetPlanet and obj.targetPlanet.colonized
                   and obj.targetPlanet.buildings
                   and stargateCooldown_ <= 0 then
                    local hasGate = false
                    for _, b in ipairs(obj.targetPlanet.buildings) do
                        if b.key == "STARGATE_NODE" then hasGate = true; break end
                    end
                    if hasGate then
                        -- 还需主曲速门（WARP_GATE_PRIME）才能触发瞬移
                        local canWarpPrime = rm_ and rm_.baseBonus
                            and rm_.baseBonus.hasWarpGatePrime
                        if canWarpPrime then
                            obj.x = obj.targetX
                            obj.y = obj.targetY
                            local def = BUILDINGS.STARGATE_NODE
                            stargateCooldown_ = (def and def.teleportCooldown) or 60
                            if notifyFn_ then
                                notifyFn_("⚡ 舰队已通过星门瞬移至 "
                                    .. (obj.targetPlanet.name or "目标星球"), "success")
                            end
                            obj.targetX = nil
                            obj.targetY = nil
                            obj.targetPlanet = nil
                            goto nextFleetObj
                        end
                    end
                end
                ...
                if d <= FLEET_ARRIVE_RADIUS then
                    ...
                    obj.targetX = nil
                    obj.targetY = nil
                    obj.targetPlanet = nil   -- P2-4: 到达后清除星门目标
...
tryClickFleet = function(mx, my)
    ...
    for i = 1, fm_.maxFleets do
        ...
        if fl and #fl.ships > 0 then
            ...
            if obj then
                local d = dist(wx, wy, obj.x, obj.y)
...
function GalaxyScene.Update(dt)
    ...
    -- P2-4: 星门冷却每帧递减
    if stargateCooldown_ > 0 then
        stargateCooldown_ = math.max(0, stargateCooldown_ - dt)
    end
